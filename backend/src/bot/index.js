...
//                 "If you need further assistance, contact polyclinic@ytech.space"
//             );
//         } catch (error) {
...
//                 ctx.session.appointmentData.purpose = ctx.message.text.trim();
...
//                     `Time: ${time}\n` +
//                     `Type: ${type}\n` +
//                     `Reason: ${ctx.session.appointmentData.purpose}\n\n` +
...
//                         const { providerId, dateTime, type, purpose } = ctx.session.appointmentData;
                        
//                         // Validate required fields
//                         if (!providerId || !dateTime || !type || !purpose) {
...
//                     message += `Specialization: ${user.expertise || 'N/A'}\n`;
...
//             try {
//                 // Fetch available expertise
//                 const response = await axios.get(
//                     `${process.env.API_URL}/providers/expertise`
//                 );

//                 const expertise = response.data.expertise;
                
//                 if (!expertise || !Array.isArray(expertise) || expertise.length === 0) {
//                     throw new Error('No expertise available');
//                 }

//                 ctx.session.step = 'appointment_specialization';
//                 ctx.session.appointmentData = {};

//                 let message = "Please select a medical expertise:\n\n";
//                 expertise.forEach((spec, index) => {
...
//                 if (error.message === 'No expertise available') {
//                     await ctx.reply(
//                         "There are currently no available expertise. Please try again later or book through our website."
//                     );
//                 } else if (error.response?.status === 401) {
//                     await ctx.reply(
//                         "Your session has expired. Please use /link to reconnect your account."
...
//                 } else {
//                     await ctx.reply(
//                         "Failed to retrieve expertise. Please try booking through our website."
//                     );
//                 }
...
//     // Additional message handler for expertise selection
...
//         try {
//             // Validate selection
//             if (isNaN(selection) || selection < 1 || selection > ctx.session.expertise.length) {
//                 await ctx.reply("Invalid selection. Please select a valid number from the list.");
//                 return;
//             }
            
//             const selectedSpecialization = ctx.session.expertise[selection - 1];
//             ctx.session.appointmentData.expertise = selectedSpecialization;
            
//             // Fetch providers with this expertise
//             const response = await axios.get(
//                 `${process.env.API_URL}/providers?expertise=${encodeURIComponent(selectedSpecialization)}`,
//                 {
//                     headers: {
//                         'Authorization': `Bearer ${ctx.session.userData.token}`
//                     }
...
//                 await ctx.reply(
//                     `No providers available for ${selectedSpecialization}. Please select another expertise or try again later.`
//                 );
//                 ctx.session.step = 'idle';
//                 return;
//             }
...
//                 message += `${index + 1}. Dr. ${provider.firstName} ${provider.lastName}\n`;
...
//             await ctx.reply(message);
//         } catch (error) {
//             console.error('Error processing expertise selection:', error.response?.data || error.message);
//             await ctx.reply(
