    ...
    <nav class="bg-white/95 backdrop-blur-md shadow-lg border-b border-medical-blue/10 sticky top-0 z-50">
                ...
                <div class="flex items-center">
                    ...
                    <router-link to="/" class="flex items-center space-x-3">
                        ...
                        <span class="text-xl font-bold">
                            POLYCLINIC.<span
                                ...
                                class="text-medical-green">.SPACE</span>
                        </span>
                    </router-link>
                    ...
                    <div class="hidden sm:ml-8 sm:flex sm:space-x-8">
                        ...
                        <router-link to="/providers"
                            class="text-gray-700 hover:text-medical-blue px-3 py-2 text-sm font-medium transition-colors duration-200"
                            :class="{ 'text-medical-blue border-b-2 border-medical-blue': $route.path === '/providers' }">
                            Find Providers
                        </router-link>
                        <template v-if="authStore.isAuthenticated">
                            <router-link v-if="authStore.isClient" to="/appointments/client"
                                class="text-gray-700 hover:text-medical-blue px-3 py-2 text-sm font-medium transition-colors duration-200"
                                :class="{ 'text-medical-blue border-b-2 border-medical-blue': $route.path.includes('/appointments/client') }">
                                My Appointments
                            </router-link>
                            <router-link v-if="authStore.isProvider" to="/appointments/provider"
                                class="text-gray-700 hover:text-medical-blue px-3 py-2 text-sm font-medium transition-colors duration-200"
                                :class="{ 'text-medical-blue border-b-2 border-medical-blue': $route.path.includes('/appointments/provider') }">
                                My Schedule
                            </router-link>
                        </template>
                    </div>
        ...
        <div v-if="showMobileMenu" class="sm:hidden bg-white/95 backdrop-blur-md border-t border-medical-blue/10">
            <div class="px-2 pt-2 pb-3 space-y-1">
                ...
                <router-link to="/providers"
                    class="text-gray-700 hover:text-medical-blue block px-3 py-2 rounded-md text-base font-medium transition-colors duration-200"
                    :class="{ 'text-medical-blue bg-medical-blue/5': $route.path === '/providers' }"
                ...
                <template v-if="authStore.isAuthenticated">
                    <router-link v-if="authStore.isClient" to="/appointments/client"
                        class="text-gray-700 hover:text-medical-blue block px-3 py-2 rounded-md text-base font-medium transition-colors duration-200"
                        :class="{ 'text-medical-blue bg-medical-blue/5': $route.path.includes('/appointments/client') }"
                        @click="closeMobileMenu">
                        My Appointments
                    </router-link>
                    <router-link v-if="authStore.isProvider" to="/appointments/provider"
                        class="text-gray-700 hover:text-medical-blue block px-3 py-2 rounded-md text-base font-medium transition-colors duration-200"
                        :class="{ 'text-medical-blue bg-medical-blue/5': $route.path.includes('/appointments/provider') }"
                        @click="closeMobileMenu">
                        My Schedule
                    </router-link>
